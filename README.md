...
```
Your App  →  OmniRoute  →  ☁️ Cloud (Groq API)
                         →  🖥️ Edge  (Local MLX / YOLOv8)
```
...
- **Automatic cloud → edge fallback** — if cloud fails, transparently retries on local models
...
| 2 | Device offline | 🖥️ **Edge** | No network available |
| 3 | Ping > 150ms | 🖥️ **Edge** | Cloud would be too slow |
| 4 | Battery < 20% and unplugged | ☁️ **Cloud** | Preserve device battery |
| 5 | Available RAM < 2GB | ☁️ **Cloud** | Not enough memory for local models |
| 6 | Ping < 50ms | ☁️ **Cloud** | Fast network, cloud gives better quality |
| 7 | Ping 50–150ms (gray zone) | ☁️ **Cloud** | Prefer cloud, edge fallback on failure |
...
SSE streaming endpoint for text (real-time token delivery).
...
5. If cloud fails → **automatic edge fallback**
